...
export class IssueboardMCP {
  ...
  updateIssueboardAgents(agentIds: string[]): PBLToolResult {
    ...
    return { success: true, message: 'Issueboard agents updated successfully.' }
  }
  ...
  createIssue(params: {
    ...
    if (parent_issue && !this.config.issueboard.issues.find(i => i.id === parent_issue)) {
      return { success: false, error: `Parent issue "${parent_issue}" not found.` }
    }
    ...
    return { success: true, issue_id: issueId, message: 'Issue created with question and judge agents.' }
  }
  ...
  updateIssue(params: {
    ...
    if (params.parent_issue !== undefined && params.parent_issue !== null &&
        !this.config.issueboard.issues.find(i => i.id === params.parent_issue)) {
      return { success: false, error: `Parent issue "${params.parent_issue}" not found.` }
    }
  ...
  activateNextIssue(): PBLToolResult {
    ...
    return { success: true, issue_id: next.id, message: `Activated issue: ${next.title}` }
  }
  ...
  completeCurrentIssue(): PBLToolResult {
    ...
    current.is_active = false
    this.config.issueboard.current_issue_id = null
    return { success: true, message: `Issue "${current.id}" marked as complete.` }
  }
}
