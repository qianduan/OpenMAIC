...
export function useCanvasOperations() {
  ...
  const activeElementList = useMemo(() => currentSlide.elements.filter(el => activeElementIdList.includes(el.id)), [currentSlide.elements, activeElementIdList]);
   ...
   */
  const updateElement = useCallback((data: UpdateElementData) => {
    const {id, props} = data;
   ...
   */
  const updateSlide = useCallback((props: Partial<Slide>) => {
    updateSceneData(draft => {
   ...
   */
  const removeElementProps = useCallback((data: RemovePropData) => {
