    ...
    for (const item of obj) {
      ...
      if ('tagName' in item && item.tagName === 'br') {
        ...
        if (bulletFlag && styleObj['list-type'] === 'ol') {
          options.bullet = { type: 'number', indent: (options.fontSize || DEFAULT_FONT_SIZE) * 1.25 }
          ...
          for (const el of slide.elements) {
            ...
            // ── CHART ──
              ...
              if (el.themeColors.length === 10) {
                ...
                chartColors = tinycolor(el.themeColors[0]).analogous(10).map(c => formatColor(c.toHexString()).color)
              }
              else {
                const len = el.themeColors.length
                const supplement = tinycolor(el.themeColors[len - 1]).analogous(10 + 1 - len).map(c => c.toHexString())
                chartColors = [...el.themeColors.slice(0, len - 1), ...supplement].map(c => formatColor(c).color)
              }
...
export function useExportPPTX() {
  ...
  const withExportGuard = useCallback((action: () => Promise<void>) => {
  ...
  }, [withExportGuard, slides, slideScenes, stage, viewportSize, viewportRatio, ratioPx2Inch, ratioPx2Pt, t])
  ...
  }, [withExportGuard, slides, slideScenes, scenes, stage, viewportSize, viewportRatio, ratioPx2Inch, ratioPx2Pt, t])
