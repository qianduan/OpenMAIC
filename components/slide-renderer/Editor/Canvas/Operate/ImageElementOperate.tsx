...
export function ImageElementOperate({ elementInfo, handlerVisible, rotateElement, scaleElement }: ImageElementOperateProps) {
  const canvasScale = useCanvasStore.use.canvasScale();
  const clipingImageElementId = useCanvasStore.use.clipingImageElementId();

  const isCliping = useMemo(() => clipingImageElementId === elementInfo.id, [clipingImageElementId, elementInfo.id]);
